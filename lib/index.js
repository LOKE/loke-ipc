...
function Communications (conf, logger) {
  this.conf = conf;
  this.channels = [];
...
Communications.$inject = ['Conf', 'Logger'];
...
Communications.prototype.$stop = function () {
  this._logger.debug('stopped Communications');
...
Communications.prototype._connect = function () {
  ...
  .then(function (connection) {
    ...
    connection.on('error', function (err) {
      if (self.onerror) return self.onerror(err);
      throw new Error('KILLING PROCESS, Received an error in rabbitmq connection: \n'+err.stack);
    });
    connection.on('close', function (err) {
      if (self.onclose) return self.onclose(err);
