...
RpcService.prototype.setupSocket = function (comm) {
  ...
  .then((ch) => {
    this.ch = ch;
    return ch.assertQueue(this.queueName, {durable: false});
  })
  .then(() => {
    return this.ch.consume(this.queueName, this._msgHandler.bind(this));
  })
  .then((ok) => {
...
RpcService.prototype._msgHandler = function (msg) {
  ...
  d.on('error', (err) => {
    this.emit('request:uncaughterr', err);
    if (this._logger) {
      this._logger.error('BAD REQUEST ERROR: ' + err.message + '\n' + err.stack, err);
    }
    d.dispose();
  });
};
...
RpcService.prototype._handleRequest = function (request) {
  return this.methods[request.method].execRequest(request) // RpcMethod#execRequest
  .then((response) => {
    return {
      request: request,
      response: response
