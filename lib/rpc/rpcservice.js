...
function RpcService (comm, serviceName, version, meta) {
  if (!serviceName) {
...
RpcService.prototype.setupSocket = function () {
  var self = this;
...
RpcService.prototype.handleMsg = function (msg) {
  var self = this;
  ...
  .then(function (response) {
    self.handleResponse(msg, response);
  })
  .fail(function (err) {
    throw err;
  }).done();
...
RpcService.prototype.handleRequest = function (request) {
  return this.methods[request.method].execRequest(request);
};

RpcService.prototype.handleResponse = function (msg, response) {
