...
Subscriber.prototype.setupSocket = function (comm) {
  ...
  .then(function (ok) {
    ...
    self.ch.consume(self.queueName, self._msgHandler.bind(self, comm), {noAck: true, exclusive: true});
  });
};

Subscriber.prototype._msgHandler = function (comm, msg) {
  var self = this;
  var d = domain.create();
...
Subscriber.prototype._unbindQueue = function(routingKey) {
  ...
  .fail(function (reason) {
    self._logger.warn('failed to unbind queue', reason);
  });
};
