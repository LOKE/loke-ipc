...
Communications.$service = 'Comm';
Communications.$inject = ['Conf'];
Communications.externalServices = {};
...
Communications.prototype.$start = function () {
  ...
  .then(function () {
    console.log('Started Communications');
  });
...
Communications.prototype.$stop = function () {
  console.log('stopped Communications');
...
Communications.prototype._connect = function () {
  ...
  var url = self.conf.get('comms:rabbit_url') || 'amqp://localhost';
  console.log('Connecting to rabbitmq at %s', url);
...
// @TODO: add command to close and kill all sockets and context
// @TODO: add command to get status (sockets connected, public interface, exposed name, general rabbit info)

// @TODO: Need to hook the rabbit.js creation of correlationIds, so that we can make it ourselves.
// correlationId let's us link requests in a chain, it'll be determined by the current domain,
// or created when a request is started if a domain doesn't exist.
// @TODO: need to set rpcID on the id field of all rpc requests/replies
// is going to be a unique uuid used by JSON-RPC to match requests and replies.
...
Communications.prototype.exposeService = function (version, serviceInstance) {
  ...
  var serviceRpc = rpc.createFromService(self, serviceInstance, version);
  var metaRpc = rpc.createRpcService(self, serviceRpc.name, version, true);
  ...
  console.log('Exposing service', serviceRpc.name);

  return Q.try(function () {
    return serviceRpc.setupSocket();
  })
  .then(function () {
    return metaRpc.setupSocket();
  });
...
Communications.prototype.getSubscriber = function () {
  if (!this._rpcSubscriberPromise) {
    this._rpcSubscriber = new pubsub.Subscriber(this);
    this._rpcSubscriberPromise = this._rpcSubscriber.setupSocket()
...
Communications.prototype.createPublisher = function (serviceName) {
  var publisher = new pubsub.Publisher(this, serviceName);

  return publisher.setupSocket()
